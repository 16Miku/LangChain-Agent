...
#### Phase 5.10.1 移除随机图片，优化纯文字布局 (P0)
...
- **现象**: 内容偏上，底部大片空白
- **原因**:
  1. `python-pptx` 无法像 HTML/CSS 那样自动计算元素实际渲染高度
  2. 文本框的高度设置无法自动收缩到内容大小
  3. 缺乏渲染后的视觉验证机制

- **pptx-skills 优势**: 使用 Playwright 渲染 HTML 后提取精确位置，真正做到"所见即所得"
...
1. **短期**: 调整居中算法参数，增加偏移量
2. **中期**: 考虑使用更激进的居中策略
3. **长期**: 研究是否引入 HTML 预渲染机制（类似 pptx-skills）
